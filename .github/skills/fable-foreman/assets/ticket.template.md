...
```json
{
  ...
  "context_paths": ["<absolute workspace path>"],
...
3. `workspace_mode` must match the baseline event. `context_paths`, `write_set`, and `output_path` are rooted in the opened workspace.
...
6. In `in_place` mode, every worker dispatch is serialized regardless of disjoint write sets.
